...
const CATEGORY_MAPPING = {
  'Еда': [
    'ДЕСЕРТЫ', 'ЕДА', 'ЗАКУСКИ', 'ВЫПЕЧКА', 'СЛАДОСТИ', 'ДЕСЕРТ'
  ],
  'Посуда': [
    'ПИАЛЫ', 'ГАЙВАНИ', 'ЧАЙНИКИ', 'ЧАХАИ', 'ЧАБАНИ', 'ЧАХЭ', 
    'СИТО', 'ВОРОНКИ-СИФОНЫ-СТАНЦИИ', 'ТЕРМОСЫ- БУТЫЛКИ-КРУЖКИ', 'НАБОРЫ',
    'ПОСУДА', 'КЕРАМИКА', 'ГЛИНА', 'ПОУДА', 'ПОСУДА'
  ],
  'Аксессуары': [
    'БЛАГОВОНИЯ', 'ФИГУРКИ', 'ИНСТРУМЕНТЫ', 'СУМКИ', 'ПОЛОТЕНЦА', 
    'ПОДСТАВКИ', 'ХРАНЕНИЕ-УПАКОВКА', 'ПРОЧЕЕ', 'АКСЕССУАРЫ', 'АКСЕССУАРЫ И ПРОЧЕЕ'
  ],
  'Услуги': [
    'УСЛУГИ', 'ЦЕРЕМОНИИ', 'МАСТЕР-КЛАССЫ', 'VIP-ЗАЛ', 'АРЕНДА'
  ],
  'Чай': [
    'МАТЭ', 'ПУЭР', 'ШУ ПУЭР', 'ШЭН ПУЭР', 'УЛУН', 'ЗЕЛЕНЫЙ ЧАЙ', 'БЕЛЫЙ ЧАЙ', 'КРАСНЫЙ ЧАЙ', 
    'ТРАВЫ', 'ТРАВЫ И ДОБАВКИ', 'ЧАЙ', '7 ПУЭР', '5 КРАСНЫЙ', '1 ЗЕЛЁНЫЙ', 'ТАЙВАНЬ', 
    'УИШАНЬ', '2 БЕЛЫЙ', '8 ТРАВЫ-ДОБАВКИ-ПРОЧЕЕ', 'МАТЧА', 'ГУАНДУН', 'ЮНЬНАНЬ', '6 ЧЁРНЫЙ', 'АНЬСИ'
  ],
};
...
function getParentCategory(subCategory) {
  ...
  if (sub.includes('ЧАЙ') || sub.includes('ПУЭР') || sub.includes('УЛУН') || sub.includes('ТРАВ')) return 'Чай';
  if (sub.includes('ПОСУДА') || sub.includes('ЧАЙНИК') || sub.includes('ПИАЛ') || sub.endsWith('УДА') || sub.includes('ПОУД')) return 'Посуда';
  if (sub.includes('ЕДА') || sub.includes('ДЕСЕРТ')) return 'Еда';
  if (sub.includes('АКСЕССУАР') || sub.includes('ПРОЧЕЕ') || sub.includes('БЛАГОВОНИ') || sub.includes('УПАКОВК') || sub.includes('ХРАНЕН')) return 'Аксессуары';
  if (sub.includes('УСЛУГ')) return 'Услуги';
